...
import { getallActors, getallMoviesById, getallProducers } from '../../redux/actions';
...
const EditMovie = () => {
  ...
  console.log("sm",singleMovie)
    let auth = localStorage.getItem("auth");
    let authuser = JSON.parse(auth);
    let token = authuser?.token;
   // console.log(token);

    const [genres, setGenres] = useState(singleMovie?.genres);
   console.log(genres)
    const [original_language, setoriginal_language] = useState(singleMovie?.original_language);
    const [originalTitle, setOriginalTitle] = useState(singleMovie?.original_title);
    const [overview, setOverview] = useState(singleMovie?.overview);
    const [poster_path, setposter_path] = useState(singleMovie?.poster_path);
    const [releaseDate, setreleaseDate] = useState(singleMovie?.releaseDate);
    const [vote_average, setvote_average] = useState(singleMovie?.vote_average);
    const [status, setstatus] = useState(singleMovie?.status);

    const [producerName, setProducerName] = useState(singleMovie?.producer.name);
  const [producerGender, setProducerGender] = useState(singleMovie?.producer.gender);
  const [producerDOB, setProducerDOB] = useState(singleMovie?.producer.dob);
  const [producerBio, setProducerBio] = useState(singleMovie?.producer.bio);
  const [producer, setproducer] = useState(singleMovie?.producer);
  const [selectedProducer, setSelectedproducer] = useState({});
  const [existingproducer, setExistingproducer] = useState(null);
  //actorStates
  const [actors, setActors] = useState([singleMovie?.actors]);
  ...
  const handleoriginal_language = (e) => {
    ...
    console.log(editformData)
        const res = await axios.put(`${API}/movies/${movieId}`, {editformData}, {
          headers: {
            Authorization: token,
          },
        });
